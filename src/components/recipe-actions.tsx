...
export function RecipeActions({
  ...
  const handleAddToGrocery = async () => {
    if (!isSignedIn) {
      setSaveLabel('Sign in to use your grocery list')
      setTimeout(() => setSaveLabel(null), 2500)
      return
    }
    if (addState !== 'idle') return
    const add = ingredients.flatMap((g) => g.items).map((s) => s.trim()).filter(Boolean)
  ...
  return (
    ...
    <div className="flex items-center gap-3 py-5 border-b border-line flex-wrap">
      ...
      >
        <PlusIcon />
        {addState === 'loading' ? 'Adding…' : addState === 'done' ? 'Added to list ✓' : 'Add to grocery list'}
      </button>
      <button
